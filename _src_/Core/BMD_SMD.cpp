...
BOOL BMD_SMD::Unpack(const char* szSrc, const char* szDest)
{
	...
	if (!szDest)
	{
		...
		strcpy_s(szNewDest, 256, fs::path(szSrc).parent_path().append(sName + "_smd").append(sName + ".smd").string().c_str());
		szDest = szNewDest;
	}
...
BOOL BMD_SMD::Pack(const char* szSrc, const char* szDest)
{
	if (!szSrc) return FALSE;
	...
	if (!szDest)
	{
		...
		strcpy_s(szNewDest, 256, fs::path(szSrc).parent_path().parent_path().append(sName).string().c_str());
		szDest = szNewDest;
	}
...
BOOL BMD_SMD::LoadLockPostionData(const char* fname)
{
...
BOOL BMD_SMD::GetLockPosition(std::string& name, short action)
{
...
BOOL BMD_SMD::SetLockPosition(std::string& name, short action)
{
	auto range = LockPositionData.equal_range(name);
	for (auto& it = range.first; it != range.second; it++)
	{
...
BOOL BMD_SMD::LoadBmd(const char* szSrc)
{
	Release();
...
BOOL BMD_SMD::ReadBmd()
{
	...
	for (int i = 0; i < m_data.NumActions; i++)
	{
		...
		if (a->LockPositions)
		{
			...
			BMD_SMD::SetLockPosition(m_data.Name + ".bmd", i);
		}
		else
		{
			a->Positions = NULL;
...
BOOL BMD_SMD::SaveSmd(const char* szDest)
{
...
BOOL BMD_SMD::Bmd2Smd(std::ofstream& os, short action)
{
...
BOOL BMD_SMD::LoadSmd(const char* szSrc)
{
	fs::path p(szSrc);
	...
	Release();
	m_data.Name = sName;
	BMD_SMD::ReadSmd(is, SMD_REFERENCE);
	is.close();
...
BOOL BMD_SMD::ReadSmd(std::ifstream& is, short action)
{
	SMD_TYPE SmdType;
	if (action == SMD_REFERENCE)
	{
	...
	std::transform(bmd_name.begin(), bmd_name.end(), bmd_name.begin(), ::tolower);
	a->LockPositions = BMD_SMD::GetLockPosition(bmd_name, action);
	std::string line;
...
BOOL BMD_SMD::SaveBmd(const char* szDest)
{
...
BOOL BMD_SMD::Smd2Bmd(BYTE version)
{
