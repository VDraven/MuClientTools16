		...
		for (int i = 0; i < m_data.NumActions; i++)
		{
			...
			auto AddAnim = [&](FbxNode* node, BoneMatrix_t* bm, vec3_t* lock_positions = NULL)
			{
				...
				for (int k = 0; k < nKeys; k++)
				{
					...
					t[0] = !bm ? 0.0 : lock_positions ? lock_positions[k][0] : bm->Position[k][0];
					t[1] = !bm ? 0.0 : lock_positions ? lock_positions[k][1] : bm->Position[k][1];
					t[2] = !bm ? 0.0 : lock_positions ? lock_positions[k][2] : bm->Position[k][2];
